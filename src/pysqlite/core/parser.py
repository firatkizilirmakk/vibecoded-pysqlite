...
class Parser:
    ...
    def parse(self, query_string):
        ...
        if query_upper.startswith('BEGIN'):
            return self._parse_begin(query_string)
        if query_upper.startswith('COMMIT'):
            return self._parse_commit(query_string)
        if query_upper.startswith('ROLLBACK'):
            return self._parse_rollback(query_string)
    ...
    def _parse_begin(self, query):
        return {'type': 'BEGIN'}

    def _parse_commit(self, query):
        return {'type': 'COMMIT'}

    def _parse_rollback(self, query):
    ...
    def _parse_from_clause(self, from_str):
        ...
        return {'type': 'join', 'join_type': join_type, 'left': {'type': 'table', 'name': left_table_str.strip()}, 'right': {'type': 'table', 'name': right_table_str.strip()}, 'on': {'left_table': t1_alias, 'left_column': t1_col, 'right_table': t2_alias, 'right_column': t2_col}}
